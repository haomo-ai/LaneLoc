...
class img_dataset_out2(Dataset):
    def __init__(self, txt, transform=None, loader=img_rgb_loader):
        ...
        for line in fh:
            ...
            line = line.rstrip()
            words = line.split()
            imgs.append((words[0], int(words[1]), int(words[2]), int(words[1])))
        self.imgs = imgs
...
def img_rgb_random_loader(path, rnd):
    step = int(rnd * 20)
    area_begin, area_end = 80, 250
    img =  cv2.imread(path)
    img = cv2.cvtColor(img,cv2.COLOR_BGR2RGB)
    begin, end = area_begin + step, area_end + step
    img =  img[begin:end, 0:480, 0:3]
    return img
...
class classifer_head(nn.Module):
    def __init__(self, in_ch, num_classes):
        ...
        self.conv1 = nn.Conv2d(in_ch, 512, kernel_size=(6, 8), stride=1, padding=0, bias=False)
...
if __name__ == '__main__':
    ...
    train_data = img_dataset_out2(txt=root + 'txt_cnn/test_data_tu.txt', transform=transforms.ToTensor())
